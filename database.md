...
<a name="the-timezone-option"></a>
#### Параметр `timezone`

Если ключ timezone не задан в config/database.php для вашей базы данных, то значение $config['timezone'] в коннекторе (например \Illuminate\Database\Connectors\PostgresConnector::class) будет равно null. 
```
protected function configureTimezone($connection, array $config)
{
    if (isset($config['timezone'])) { // Тут будет null
...
В результате сервер базы данных будет использовать часовой пояс сервера, а не приложения. Это может быть проблемой. Если сервер настроен неверно, то приложение пытается сохранить дату 12-12-2025T12:12:12+03, а в базу сохранится например 12-12-2025T12:12:12+00 (если timezone сервера UTC). Без этого параметра нужно следить за временной зоной серверов.
Существование ключа config('database.dbname.timezone') нигде не задокументировано, и его влияние оставалось незамеченным до обновления до версии 11. Коннектор смотрит именно на config/database.php config/app.php timezone он игнорирует.
